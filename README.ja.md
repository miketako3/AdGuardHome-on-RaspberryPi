...
4. SDカードのブートパーティションがローカルにマウントされており、Raspberry Pi Imager が生成した `user-data` が存在すること。
...
6. SDカードの既存 Imager `user-data` に対して設定をマージ:
...
2. `http://<raspberry-pi-ip>/` にアクセスし、AdGuard Home の管理画面が開くこと。

## Make ターゲット

- `make render`: `templates/user-data.tmpl` から `build/user-data` を生成
- `make install`: Imager が作成した `${BOOT_DIR}/user-data` を保持しつつ、multipart 形式でマージして `${BOOT_DIR}/user-data` に書き戻し
...
- `scripts/merge_user_data.sh`: Imager設定を保持したまま multipart `user-data` を生成
...
- `make install` 初回実行時に、Imager 元設定を `${BOOT_DIR}/user-data.imager.orig` にバックアップします。
...
- `ERROR: ADGUARD_ADMIN_USER is required`: `.env` に `ADGUARD_ADMIN_USER` を設定してください。
- `ERROR: ADGUARD_ADMIN_PASSWORD is required`: `.env` に `ADGUARD_ADMIN_PASSWORD` を設定してください。
- `ERROR: ADGUARD_DNS_BIND_HOST is required`: `.env` に `ADGUARD_DNS_BIND_HOST` を設定してください。
- `ERROR: htpasswd is required`: `apache2-utils`（Linux）または `httpd`（macOS Homebrew）を導入してください。
- `ERROR: BOOT_DIR does not exist`: SDカードのブートパーティションのマウント先と `BOOT_DIR` を確認してください。
- `ERROR: /.../user-data not found`: 先に Raspberry Pi Imager でOS書き込みを行い、初期 cloud-init ファイルを作成してください。
- 書き込み直後に `BOOT_DIR` エラーになる: SDカードを抜き差ししてから、`/Volumes` 配下のマウント先を再確認してください。
- 以前の実行で SSH 設定が消えた: 一度 SD カードを再イメージしてから、このマージ方式の `make install` を実行してください。
- 初回起動直後に `tailscaled.service` が見つからない:
  多くの場合、`cloud-init` が長い初回アップグレード処理中で `scripts-user` まで到達していない状態です。
  本リポジトリでは、Tailscale/AdGuard 起動後に `apt-get upgrade` を最後に実行する順序へ変更して、この遅延を回避しています。
